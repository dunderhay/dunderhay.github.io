...
You can now type commands, similar to how you would interact with an FTP server. Issue the `HELP` command (case sensitive) to see what functions are supported:

<img src="/assets/images/vulnserver/intro/vulnserver-help-command.png" alt="vulnserver-commands" width="450"/>

Looking back at the Windows host, the vulnserver application has received a connection from our attacker machine:

<img src="/assets/images/vulnserver/intro/vulnserver-new-connection.png" alt="vulnserver-new-connection" width="550"/>

It is possible to write a python script using sockets and the socket API to send different commands via a TCP packets to the vulnserver application. For example:
...
*Note: This will **not** crash the vulnserver application.*

The `TRUN` command ran successfully with the following output: `TRUN COMPLETE`.
